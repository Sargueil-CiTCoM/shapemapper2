...
def run(args):
    ...
    try:
        ap.check_help_version(
            args[1:]
        )  # print version or usage and exit if those args provided
        ap.check_no_whitespace(
            args[1:]
        )  # check for disallowed whitespace characters in input arguments
        logpath, rest_args = ap.get_log_path(args[1:])

        log = Logger(filepath=logpath, stream=sys.stdout)
        ts = timestamp()
        term_width, term_height = shutil.get_terminal_size()
        print("\n" + "#" * term_width)
        print(
            "\nStarted ShapeMapper v{} at {}\nOutput will be logged to {}".format(
                version(), ts, logpath
            )
        )
        log.file.write("\n" + "#" * term_width)
        ...
        if pipeline.render_flowchart:
            ...
            s = draw_flowchart(
                pipeline, p, path=pipeline.temp, highlight_dir=pipeline.out
            )
            if s:
                print("Rendered pipeline flowchart to {} at {}".format(p, timestamp()))
            else:
                msg = "Attempted to render pipeline flowchart to {} at {}, "
                msg += "but graphviz gave a warning or error."
                msg = msg.format(p, timestamp())
                print(msg)
        ...
        if success:
            ...
            if len(failed_targets) == 0:
                print(
                    "ShapeMapper run successfully completed at {}.".format(timestamp())
                )
            else:
                print("ShapeMapper run completed at {}.".format(timestamp()))
                if len(failed_targets) == 1:
                    msg = (
                        "WARNING: This RNA has a possible poor quality reactivity profile: "
                        + failed_targets[0]
                    )
                    print(msg)
                else:
